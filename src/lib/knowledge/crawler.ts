...
/**
 * Probe a tenant domain and resolve its canonical (post-redirect) origin.
 *
 * Stored domains are often the apex ("doggo.com.au") but the live site lives
 * on www. We probe with redirects followed and return the final URL's origin
 * so the same-origin filter and downstream URL handling are consistent.
 *
 * Exposed for the sync orchestrator (K-04) which needs the canonical origin
 * to filter the sitemap URL queue.
 */
...
/**
 * Public wrapper around the internal sitemap fetcher, used by the sync
 * orchestrator. Takes the canonical base URL (post-redirect resolution).
 */
export async function fetchSitemapFromBase(baseUrl: string): Promise<string[]> {
  return fetchSitemap(baseUrl);
}

/**
 * Public wrapper around the internal single-page fetcher, used by the sync
 * orchestrator for per-URL processing.
 */
export async function fetchSinglePage(
  url: string,
  origin: string
): Promise<CrawledPage | null> {
  return fetchPage(url, origin);
}
...
export async function crawlSite(
  ...
  // Resolve canonical origin (apex -> www, etc.)
