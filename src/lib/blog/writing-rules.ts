...
export function validateWordCountGates(
  ...
  if (stats.totalWordCount < stats.minTotalWordCount) {
    return {
      code: "word_count",
      message: `Word-count quality gate failed: article body has ${stats.totalWordCount} intro and paragraph words, below the target minimum of ${stats.minTotalWordCount}.`,
      stats,
    };
  }
