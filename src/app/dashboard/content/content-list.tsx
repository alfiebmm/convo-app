...
export function FailedGenerationFilterChip({
  ...
  return (
    <button
      type="button"
      onClick={onToggle}
      className={`inline-flex flex-wrap items-center justify-center gap-2 rounded-lg border px-3 py-2 text-sm font-medium transition-colors ${
        includeFailed
    ...
    >
      <span>
        {includeFailed
          ? "Hiding failed generations"
          : "Include failed generations"}
      </span>
      <BlogPostStatusPill status="generation_failed" />
      <span className="ml-0 text-xs text-slate-500 sm:ml-2">
        {failedCount} archived
      </span>
    </button>
  );
}
